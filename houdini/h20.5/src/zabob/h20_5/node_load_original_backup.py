...
def load_houdini_file(file_path):
    ...
    try:
        ...
        print("Analyzing parameters and node references...")
        all_node_refs = {}
        ...
        print(f"Detected {len(chains)} linear chains")
        print(f"Found {len(all_node_refs)} nodes with parameter references")
        ...
        for c in categories:
            ...
            print(f"    node = _parent.get().createNode(node_type, name)")
            print(f"    for parm_name, value in kwargs.items():")
            print(f"        if node.parm(parm_name):")
            print(f"            node.parm(parm_name).set(value)")
            print(f"    return node")
        print()
        ...
        print("# Create nodes in topological order")
        for chain in sorted_chains:
            ...
            if parent_node != ROOT_NODE and parent_node in parents_used:
                print(f'with parent({good_var(parent_node)}):')
        ...
        print("# Explicit connections (reverse/branch connections)")
...
def extract_non_default_parms(node: hou.Node) -> dict[str, Any]:
    ...
    try:
        ...
        print(f"Warning: Could not extract parameters for {node.path()}: {e}")
...
def find_node_references_in_parms(node: hou.Node, all_nodes: set[hou.Node]) -> dict[str, list[hou.Node]]:
    ...
    try:
        ...
        print(f"Warning: Could not analyze node references for {node.path()}: {e}")
